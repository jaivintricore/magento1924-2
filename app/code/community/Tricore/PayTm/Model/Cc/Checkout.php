<?php
/**
 * PayTm Checkout Model
 */
class Tricore_PayTm_Model_Cc_Checkout {
    ...
    public function place($request) {
		...
		$res_amt=$request['TXNAMOUNT'];//rupee
		$paytmCurrencyCode="INR";
		$baseCurrencyCode = Mage::app()->getStore()->getBaseCurrencyCode();//get store base currency code
		$allowedCurrencies = Mage::getModel('directory/currency')->getConfigAllowCurrencies();
		$rates = Mage::getModel('directory/currency')->getCurrencyRates($baseCurrencyCode, array_values($allowedCurrencies));
		$price=$res_amt/$rates[$paytmCurrencyCode];
		$amount=number_format($price, 2, '.', '');
		//save order transaction detail with paytm transaction id
		$ptxn="N/A";
		$payment = $order->getPayment();
		$payment->setTransactionId($request['TXNID'])
				->setCurrencyCode($order->getBaseCurrencyCode())
				->setPreparedMessage("Paytm Order Sucess")
				->setParentTransactionId($ptxn)
				->setShouldCloseParentTransaction(true)
				->setIsTransactionClosed(0)
				->registerCaptureNotification($amount);             
		$order->sendNewOrderEmail();
		$order->setState(Mage_Sales_Model_Order::STATE_PROCESSING, true);//set order state fraud to processing
		$order->setStatus('processing', false);//set order status processing 
		$order->save();
		$this->_order = $order;
		}
